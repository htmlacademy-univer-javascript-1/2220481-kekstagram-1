...
const reValue = () => {
  ...
  if(scaling <= Scale.MAX && scaling >=  Scale.MIN){
    imageUploadPreview.style.transform = `scale(${scaling.toFixed(2)})`;
  }
};

const onButtonClickScaling =(evt) => {
  const targetImage = evt.target;
  imageUploadPreview.style.transform = `scale(${scaling})`;
...
const restart = () => {
  ...
  scaleValue.removeEventListener('change', reValue);
  scaling = Scale.START;
};
